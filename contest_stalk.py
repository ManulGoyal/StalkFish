    ...
    async def stalk(self):
        while True:
            ...
            async with CFApi() as cf_api:
                ...
                if not Contest.objects(contest_id=contests[0]['id']):
                    new_contest = Contest(contest_id=contests[0]['id'])
                    new_contest.save()
                    rating_changes = await cf_api.get_rating_changes(contests[0]['id'])
                    ...
                    if len(desired_rating_changes) > 0:
                        ...
                        embed = get_contest_embed(desired_rating_changes, contests[0]['id'],
                                                  contests[0]['name'], top=3)
